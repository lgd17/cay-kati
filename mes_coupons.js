...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("couponEdit_")) {
    ...
    return bot.sendMessage(chatId, `✏️ Edition du coupon ${id} (${canal})\n\nEnvoie la nouvelle date (YYYY-MM-DD) ou /skip pour garder.`);
  }
});
...
bot.onText(/\/skip/, async (msg) => {
  ...
  if (session.step === "await_content") {
    ...
    await pool.query(
      `UPDATE ${session.table} SET content=$1, media_type=$2, media_url=$3, schedule_date=$4, schedule_time=$5 WHERE id=$6`,
